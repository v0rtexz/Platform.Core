...
public class Charm
{
    ...
    private static extern void DirectOverlaySetup(
        [MarshalAs(UnmanagedType.FunctionPtr)] CallbackUnmanaged callBackUnmanaged, IntPtr hWnd);
    ...
    public delegate void CallbackManaged(RPM rpm, Draw draw, int width, int height);
    ...
    private void CharmCallback(int width, int height)
    {
        ...
        this._draw.SetWorldToScreenSize(width, height);

        // we essentially forward the callback the the managed user-defined callback function
        this.callbackManaged(this.rpm, this._draw, width, height);
    }
    ...
    public class RPM
    {
        ...
        private static extern bool VirtualProtectEx(IntPtr hProcess, IntPtr lpAddress, UInt32 dwSize, uint flNewProtect,
            out uint lpflOldProtect);

        [DllImport("kernel32.dll", SetLastError = true)]
        private static extern bool ReadProcessMemory(IntPtr hProcess, Int64 lpBaseAddress, [In, Out] byte[] lpBuffer,
            UInt64 dwSize, out IntPtr lpNumberOfBytesRead);

        [DllImport("kernel32.dll")]
        private static extern bool WriteProcessMemory(IntPtr hProcess, Int64 lpBaseAddress, [In, Out] byte[] lpBuffer,
            UInt64 dwSize, out IntPtr lpNumberOfBytesWritten);

        [DllImport("ntdll.dll")]
        private static extern bool NtWriteVirtualMemory(IntPtr hProcess, Int64 lpBaseAddress, [In, Out] byte[] lpBuffer,
            UInt64 dwSize, out IntPtr lpNumberOfBytesWritten);
        ...
        {
            byte[] Buffer = BitConverter.GetBytes((char)_Value);
            return WriteMemory(addr, Buffer);
        }
    ...
    public class Draw
    {
        ...
        // import DrawLine
        [DllImport(UnmanagedFileName, CallingConvention = CallingConvention.Cdecl)]
        private static extern void DrawLine(float x1, float y1, float x2, float y2, float thickness, float r, float g,
            float b, float a = 1);

        // import DrawBox
        [DllImport(UnmanagedFileName, CallingConvention = CallingConvention.Cdecl)]
        private static extern void DrawBox(float x, float y, float width, float height, float thickness, float r,
            float g, float b, float a, bool filled);

        // import DrawCircle
        [DllImport(UnmanagedFileName, CallingConvention = CallingConvention.Cdecl)]
        private static extern void DrawCircle(float x, float y, float radius, float thickness, float r, float g,
            float b, float a, bool filled);

        // import DrawEllipse
        [DllImport(UnmanagedFileName, CallingConvention = CallingConvention.Cdecl)]
        private static extern void DrawEllipse(float x, float y, float width, float height, float thickness, float r,
            float g, float b, float a, bool filled);
        ...
        private static extern void DrawString(string str, float fontSize, float x, float y, float r, float g, float b,
            float a = 1);
        ...
        public static void DrawLine(float x1, float y1, float x2, float y2, float thickness, System.Drawing.Color color)
        {
            Draw.DrawLine(x1, y1, x2, y2, thickness, ByteToFloat(color.R), ByteToFloat(color.G),
                ByteToFloat(color.B));
        }

        public static void DrawBox(float x, float y, float width, float height, float thickness, System.Drawing.Color color,
            bool filled)
        {
            Draw.DrawBox(x, y, width, height, thickness, ByteToFloat(color.R), ByteToFloat(color.G),
                ByteToFloat(color.B), ByteToFloat(color.A), filled);
        }

        public static void DrawCircle(float x, float y, float radius, float thickness, System.Drawing.Color color, bool filled)
        {
            Draw.DrawCircle(x, y, radius, thickness, ByteToFloat(color.R), ByteToFloat(color.G),
                ByteToFloat(color.B), ByteToFloat(color.A), filled);
        }

        public static void DrawEllipse(float x, float y, float width, float height, float thickness,
            System.Drawing.Color color, bool filled)
        {
            Draw.DrawEllipse(x, y, width, height, thickness, ByteToFloat(color.R), ByteToFloat(color.G),
                ByteToFloat(color.B), ByteToFloat(color.A), filled);
        }

        public static void DrawString(float x, float y, string text, System.Drawing.Color color, float fontsize = 24)
        {
            Draw.DrawString(text, fontsize, x, y, ByteToFloat(color.R), ByteToFloat(color.G), ByteToFloat(color.B));
        }
        ...
        public bool WorldToScreen(System.Numerics.Vector3 m_World, out System.Numerics.Vector3 m_Screen)
        {
            ...
            float ScreenW = (this.pViewProj.M14 * m_World.X) + (this.pViewProj.M24 * m_World.Y) +
                            (this.pViewProj.M34 * m_World.Z + this.pViewProj.M44);

            if (ScreenW < 0.0001f)
                return false;

            float ScreenX = (this.pViewProj.M11 * m_World.X) + (this.pViewProj.M21 * m_World.Y) +
                            (this.pViewProj.M31 * m_World.Z + this.pViewProj.M41);
            float ScreenY = (this.pViewProj.M12 * m_World.X) + (this.pViewProj.M22 * m_World.Y) +
                            (this.pViewProj.M32 * m_World.Z + this.pViewProj.M42);
